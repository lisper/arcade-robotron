...
unsigned RDIO (unsigned addr);
void WRIO (unsigned addr, unsigned data);
...
unsigned sc1_constant_value;
unsigned sc1_src_base;
unsigned sc1_dst_base;
unsigned sc1_width;
unsigned sc1_height;
unsigned sc1_control;

unsigned sc1_xcount;
unsigned sc1_ycount;
unsigned sc1_src_address;
unsigned sc1_dst_address;

#define sc1_suppress_upper	(sc1_control & 0x80)
#define sc1_suppress_lower	(sc1_control & 0x40)
#define sc1_shift_right		(sc1_control & 0x20)
#define sc1_constant_substitution (sc1_control & 0x10)
#define sc1_zero_write_suppress	(sc1_control & 0x08)
#define sc1_synchronize_e	(sc1_control & 0x04)
#define sc1_span_dst		(sc1_control & 0x02)
#define sc1_span_src		(sc1_control & 0x01)

void sc1_write(unsigned addr, unsigned data)
{
  int off = addr & 0x00ff;
  unsigned d, v;
    ...
    case 0:
      ...
      sc1_xcount = 0;
      sc1_ycount = 0;
      sc1_src_address = sc1_src_base;
      sc1_dst_address = sc1_dst_base;
      
      printf("SC1: start src=%04x, dst=%04x w=%d h=%d ctrl=%02x\n",
	     sc1_src_base, sc1_dst_base, sc1_width, sc1_height, sc1_control);

      while (1) {
	printf("SC1: src=%04x dst=%04x xc=%d yc=%d\n",
	       sc1_src_address, sc1_dst_address, sc1_xcount, sc1_ycount);

	d = RDMEM(sc1_src_address);
	v = sc1_constant_substitution ? sc1_constant_value : d;
	WRMEM(sc1_dst_address, v);
	printf("SC1: blt %04x <= %02x\n", sc1_src_address, v);

	if (sc1_xcount == sc1_width) {
	  sc1_xcount = 0;
	  sc1_ycount++;

	  if (sc1_ycount == sc1_height)
	    break;

	  if (sc1_span_src)
	    sc1_src_address = sc1_src_base + sc1_ycount;
	  else
	    sc1_src_address++;

	  if (sc1_span_dst)
	    sc1_dst_address = sc1_dst_base + sc1_ycount;
	  else
	    sc1_dst_address++;
	} else {
	  sc1_xcount++;
      ...
      break;
    case 1:
      sc1_constant_value = data;
      break;
    case 2:
      sc1_src_base = (sc1_src_base & 0x00ff) | ((data << 8) & 0xff00);
      break;
    case 3:
      sc1_src_base = (sc1_src_base & 0xff00) | (data & 0x00ff);
      break;
    case 4:
      sc1_dst_base = (sc1_dst_base & 0x00ff) | ((data << 8) & 0xff00);
      break;
    case 5:
      sc1_dst_base = (sc1_dst_base & 0xff00) | (data & 0x00ff);
      break;
    case 6:
      sc1_width = data ^ 0x04;
      break;
    case 7:
      sc1_height = data ^ 0x04;
      break;
    }
}
...
{
  unsigned r = 0;
  if (1) printf("RDIO[%x] => %x\n", addr, r);
  return r;
}

void WRIO (unsigned addr, unsigned data)
{
  if (1) printf("WRIO[%x] <= %x\n", addr, data);

  // Color table: write: C000-C3FF
  if ((addr & 0xfc00) == 0xc000) {
  }

  // Widget PIA: read/write: C8X4 - C8X7
  if ((addr & 0xff0c) == 0xc804) {
  }
    
  // ROM PIA: read/write: C8XC - C8XF
  if ((addr & 0xff0c) == 0xc80c) {
  }

  // Control address: write: C9XX
  if ((addr & 0xff00) == 0xc900) {
  }

  // Special chips: read/write? CAXX
  if ((addr & 0xff00) == 0xca00) {
    sc1_write(addr, data);
    return;
  }
  ...
  if ((addr & 0xfffe) == 0xcbfe) {
  }
   
  // CMOS "nonvolatile" RAM: read/write: CC00 - CFFF
  if ((addr & 0xfe00) == 0xcc00) {
  }
}

static void WRMEM (unsigned addr, unsigned data)
{
  if (addr >= 0xc000 && addr <= 0xcffff) {
    WRIO(addr, data);
    return;
  }
  if (1) printf("MEM[%x] <= %x\n", addr, data);
...
int cpu_execute (int cycles)
{
  ...
  do
  {
    if (check_break(PC) != 0) monitor_on = 1;
