...
char *suffix;
{
  ...
  if ((postbyte & 0x80) == 0x00) {
	 ...
	 case 0x08 :
		...
		printf("%0.2X %0.2X %0.2X    %s%s       %s$%0.2X,%s",
	 ...
	 case 0x09 :
		...
		printf("%0.2X %0.2X %0.2X %0.2X %s%s       %s$%0.4X,%s",
	 ...
	 case 0x0c :
		...
		s = "<";
		printf("%0.2X %0.2X %0.2X    %s%s       %s$%0.2X,PCR",
				  code, postbyte, prog[pc+2], suffix, op->name, s, offset);
		extrabytes = 1;
		break;
	 case 0x0d :
		...
		s = ">";
		printf("%0.2X %0.2X %0.2X %0.2X %s%s       %s$%0.4X,PCR",
				  code, postbyte, prog[pc+2], prog[pc+3], suffix, op->name, s, offset);
		extrabytes = 2;
		break;
	 case 0x0e :
...
void main(int argc, char **argv )
{
  ...
  if ( argc != 4 ) {
    fprintf( stderr, "usage: disasm <file> <start> <end>\n");
  ...
  size = read(fd, &prog[start], 0xffff);
